...
data_name_list = [r"$m=1$",r"$m=2$",r"$m=3$",r"$m=4$"]
...
folder = os.path.join(get_controller_path(), "sim_alg/test_online/log-test_online_static/test_online_static-2024-December-13-15-40-57-ail")

data_list = []

for i in range(5):
    ne_file = os.path.join(folder,"ES_GGM.npe.final.txt")
    ne = np.genfromtxt(ne_file, delimiter=',')[:,3:]
    ne = ne.squeeze()
    data_list.append(ne[i::5])


for i in range(5):
    nf_file = os.path.join(folder,"ES_GGM.nf.final.txt")
    nf = np.genfromtxt(nf_file, delimiter=',')[:,3:]
    nf = nf.squeeze()
    data_list.append(nf[i::5])

    
lines = []

xlabels = [r'$|\mathcal{P}^{(m)}|$',r'$\Sigma_{k}  \mathbf{1}_{\{r_k < \hat{r}\}} $']
for i in range(2):
    for j in range(4):
        data_sorted = np.sort(data_list[j+i*5])
        print(data_sorted)
        cdf = np.arange(1, len(data_sorted) + 1) / len(data_sorted)
        line, = axs[i].plot(data_sorted,cdf,linewidth=1.5,markerfacecolor='none')
        lines.append(line)
    ...
    if i == 0:
        axs[i].set_xlim(100000, 150000)
        axs[i].set_xticks([100000,125000,150000])     
    if i == 1:
        axs[i].set_xlim(0, 50)
