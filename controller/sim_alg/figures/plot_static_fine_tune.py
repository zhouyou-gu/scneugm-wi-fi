...
data_name_list = [r"$m=1$",r"$m=3$",r"$m=5$",r"$m=9$"]
...
folder = os.path.join(get_controller_path(), "sim_alg/test_online/log-test_online_static/test_online_static-2024-December-14-12-10-32-ail")

data_list = []

for i in range(11):
    ne_file = os.path.join(folder,"ES_GGM.nc.final.txt")
...
for i in range(2):
    idx = [0,2,4,8]
    for j in range(4):
        data_sorted = np.sort(data_list[idx[j]+i*11])
        print(data_sorted)
